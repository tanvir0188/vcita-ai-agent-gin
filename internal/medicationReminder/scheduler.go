	...
	for _, client := range toProcess {
		...
		go func() {
			...
			if resp != nil && resp.Response != nil {
				if _, err := utils.CreateMessage(clientCopy.UID, *resp.Response); err != nil {
...
// Schedule the daily medication reminder task. Call this from the main server startup.
...
// 	// Run every minute.
// 	c.AddFunc("*/1 * * * *", func() {
// 		log.Printf("[MedicationReminder] scheduled run starting")
// 		if err := ProcessMedicationReminders(db, store); err != nil {
// 			log.Printf("[MedicationReminder] scheduled run error: %v", err)
// 		} else {
// 			log.Printf("[MedicationReminder] scheduled run completed successfully")
// 		}
// 	})
// 	c.Start()
// }

// ProcessMedicationReminders runs the medication‑refill prediction for all clients that
// have current medications. It respects the ClientSyncState table to avoid re‑processing
// the same client more than once per day, unless a partial reminder is required.
// The function processes clients in parallel with a maximum of 10 concurrent workers.
