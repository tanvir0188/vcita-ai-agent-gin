...
// GetServices fetches the list of services from vcita.
...
// GetAvailabilitySlots fetches availability slots for a service.
func GetAvailabilitySlots(
	...
	staffIDs, err := vcita.GetStaffIDs()
	if err != nil {
		return nil, err
	}
	...
	if err := vcita.DoRequest(http.MethodGet, endpoint, headers, &response); err != nil {
		return nil, err
	}
...
// BookAppointment books an appointment via vcita API.
func BookAppointment(token string, req BookingRequest) (*BookingDetails, error) {
	...
	err := vcita.DoRequestWithBody("POST", url, headers, req, &apiResponse)
	if err != nil {
